...
If[ND===1,Do[m2=M-m1;If[-1<=m2<=1,ope+=ClebschGordan[{1,m1},{1,m2},{J,M}]Subscript[V, m1] Subscript[D, m2]],{m1,-1,1}]];
If[ND===2,Do[Do[m3=M-mD;m2=mD-m1;If[-1<=m2<=1 && -1<=m3<=1,ope+=ClebschGordan[{1,m3},{JD,mD},{J,M}]ClebschGordan[{1,m1},{1,m2},{JD,mD}]Subscript[V, m3]Subscript[D, m1]**Subscript[D, m2]],{m1,-1,1}],{mD,-JD,JD}]];
];
If[TG==="S" || TG==="P",
If[ND===0,If[J===0,ope=S]];
If[ND===1,If[J===1,ope=S Subscript[D, M]]];
If[ND===2,Do[m2=M-m1;If[-1<=m2<=1,ope+=ClebschGordan[{1,m1},{1,m2},{J,M}]S Subscript[D, m1]**Subscript[D, m2]],{m1,-1,1}]];
];
If[TG==="A",ope=ope//.{V->A}];
If[TG==="P",ope=ope//.{S->P}];
Return[ope];
